# Documentação do Diagrama de Contexto C4 - Plataforma GO
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Plataforma GO** | Sistema Central (Core) | Azul (`System`) | Plataforma estadual de saúde responsável por centralizar, processar, integrar e distribuir o ecossistema de dados de saúde pública do Estado de Goiás. Funciona como hub centralizador de interoperabilidade. | **Núcleo do Sistema** | REST / FHIR / Web Services |
| **Operador** | Ator Humano (Suporte / TI) | Cinza (`Person_Ext`) | Profissional técnico responsável pela administração, parametrização, sustentação, monitoramento operacional de tráfego, gestão de falhas e auditoria de acessos da plataforma. | Bidirecional ($\leftrightarrow$) | HTTPS / Painel Web |
| **Aplicação Clínica** | Sistema Cliente / Consumidor | Cinza (`System_Ext`) | Sistemas de atendimento e registro assistencial (prontuários, diagnósticos e prescrições) e faturamento de estabelecimentos de saúde. Alimenta e consulta o histórico clínico. | Bidirecional ($\leftrightarrow$) | HTTPS / REST / FHIR |
| **Aplicação Administrativa** | Sistema Cliente / Consumidor | Cinza (`System_Ext`) | Sistema para gestão operacional, faturamento e regulação de estabelecimentos de saúde. | Bidirecional ($\leftrightarrow$) | HTTPS / REST |
| **API SISCAN (Simulada)** | Sistema Externo (Nacional) | Cinza (`System_Ext`) | Sistema de Informação do Câncer (Ministério da Saúde), representado por um simulador com contrato nativo REST/JSON. Recebe requisições de exames e envia resultados/laudos oncológicos. | Bidirecional ($\leftrightarrow$) | HTTPS / REST / JSON |
...
1. **Aplicações Locais (Clínica e Administrativa):**
   - As **Aplicações Clínicas** enviam boletins de atendimento, prontuários e registros assistenciais e consultam a linha de cuidado unificada do paciente na **Plataforma GO**.
   - As **Aplicações Administrativas** enviam relatórios operacionais e solicitações de faturamento, recebendo confirmações de sincronização e relatórios consolidados.
...
3. **Interoperabilidade com Governo Federal e Outros Estados:**
   - **API SISCAN $\leftrightarrow$ Plataforma GO:** Fluxo bidirecional intermediado pelo Adaptador FHIR-SISCAN da Plataforma GO, permitindo o envio de solicitações/requisições de exames e o recebimento de resultados e laudos oncológicos (mamografia, citopatológico, histopatológico) do simulador da API SISCAN.
...
System_Ext(appClinica, "Aplicação Clínica", "Sistema de atendimento e registro assistencial (prontuários, diagnósticos e prescrições), faturamento de estabelecimentos de saúde.")
System_Ext(appAdmin, "Aplicação Administrativa", "Sistema para gestão operacional.")

' Sistemas Externos (Em Cinza - Ext)
System_Ext(siscan, "API SISCAN (Simulada)", "Sistema de Informação do Câncer (Ministério da Saúde), representado por simulador com contrato nativo REST/JSON.")
